...
| `DAYOFMONTH(x)`                 | Return day of the month.                                                                                                      | `x` in {`date`, `timestamp`}, returns `integer` |
| `DAYOFWEEK(x)`                  | Return day of the week (Sunday=1, Saturday=7).                                                                                | `x` in {`date`, `timestamp`}, returns `integer` |
| `DAYOFYEAR(x)`                  | Return day number in the year.                                                                                                | `x` in {`date`, `timestamp`}, returns `integer` |
| `EPOCH(x)`                      | Return seconds since the Unix epoch with microsecond precision.                                                               | `x` in {`date`, `timestamp`}, returns `double`  |
| `EXTRACT(FIELD FROM x)`         | Return the specified `FIELD` from the date value `x`. Fields include `YEAR`, `MONTH`, `DAYOFMONTH`, `HOUR`, `MINUTE`, `SECOND`, `EPOCH`, etc. | `x` in {`date`, `time`, `timestamp`}, returns `integer` or `double` |
...
| `HOUR(x)`                       | Return hour (in 24-hour format).                                                                                              | `x` in {`time`, `timestamp`}, returns `integer` |
...
| `MONTHNAME(x)`                  | Return name of the month in the default locale.                                                                               | `x` in {`date`, `timestamp`}, returns `string` |
...
| `QUARTER(x)`                    | Return the quarter.                                                                                                           | `x` in {`date`, `timestamp`}, returns `integer` |
...
| `WEEK(x)`                       | Return the week of the year (1-53).                                                                                           | `x` in {`date`, `timestamp`}, returns `integer` |
| `YEAR(x)`                       | Return the four-digit year.                                                                                                   | `x` in {`date`, `timestamp`}, returns `integer` |
...
|             | - `SQL_TSI_YEAR` - years                                                                                                                                                                                                        |
| `count`     | A long or integer count of units to add to the timestamp. Negative values subtract that number of units. Long values are allowed for symmetry with `TIMESTAMPDIFF`, but the effective range is still limited to integer values. |
| `timestamp` | A datetime expression.                                                                                                                                                                                                          |

Example:
```sql
SELECT TIMESTAMPADD(SQL_TSI_MONTH, 12,'2025-10-10');
SELECT TIMESTAMPADD(SQL_TSI_SECOND, 12,'2025-10-10 23:59:59');
```
...
Please note that:
- If `endTime > startTime`, a non-negative number will be returned.  
...
| Function                 | Description                                                                                              | Datatype constraint                         |
|--------------------------|----------------------------------------------------------------------------------------------------------|---------------------------------------------|
| `vectorStringToArray(x)` | Transforms a string that represents a vector, in the form `[0.1, 0.2, 0.3,...]` to an array of `double`. | x must be string.                           |
| `euclideanDistance(x,y)` | Computes the **euclidean** distance of x and y                                                           | x, y must be `double[]` with the same size. |
| `innerProduct(x,y)`      | Computes the **inner product** (or dot product) of x and y                                               | x, y must be `double[]` with the same size. |
| `cosineDistance(x,y)`    | Computes the **cosine** distance of x and y                                                              | x, y must be `double[]` with the same size. |
