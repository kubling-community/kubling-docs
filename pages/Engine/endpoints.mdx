...
`get_k8s_component_info` and `find_best_cluster_for_deploy` are [user-defined template functions](/Modules/Functions/Template). 
The function `find_best_cluster_for_deploy` is particularly important because it returns the name of the optimal cluster schema 
...
Although the operation is idempotent, the user-defined template function `ns_exists`, which returns a `boolean` indicating whether the namespace already exists, 
