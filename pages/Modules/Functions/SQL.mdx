...
That simple functions receives an Azure ID, and returns the Resource Group, which is located in the fourth position of the FQN.

The argument passed to functions is always one `args`, which is a KV with three members:
- `value`: the document field OR the column value specified in the Directive, by enclosing it in &lt;&gt;.
