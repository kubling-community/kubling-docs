...
and embedded systems. It allows querying and mutation of managed device state via lightweight, request-response messages over UDP and TCP.

At the core of SNMP is the **OID** (Object Identifier): a dot-separated numeric path that uniquely identifies a managed value exposed by a device. 
For example, the OID `1.3.6.1.2.1.1.5` typically corresponds to the system's hostname (`sysName`). 
These identifiers are part of a global, hierarchical tree managed by IANA and extended by device vendors.

Devices expose OIDs through a **Management Information Base (MIB)**, a virtual schema that defines the structure, data types, access modes, 
and semantics of available managed values. 
Most MIBs are vendor-specific extensions of standardized SNMP trees.

SNMP organizes managed data into **scalar values** and **tables**, where tables represent collections of related 
managed instances (for example, network interfaces, physical components, or power states). 
Many real-world use cases rely on iterating full tables to inspect or modify device configuration and operational state.

Kubling models SNMP-managed data through a relational abstraction, allowing SNMP tables and scalar values to be represented using `CREATE FOREIGN TABLE` statements. 
This abstraction preserves the underlying SNMP semantics: rows correspond to managed instances, columns map to individual OIDs, 
and operations reflect the capabilities and constraints of the SNMP protocol.


<Callout type='warning'>
As of version `26.1`, only **UDP** connections are supported for SNMP.  
We are working to add **TCP** support, so expect it in a future release.
</Callout>

## Supported SQL Operations

The SNMP Data Source supports a constrained subset of SQL operations that directly reflect the capabilities of the SNMP protocol.

### SELECT

`SELECT` operations map to SNMP `GET` and `WALK` operations.

They are used to retrieve the current value of scalar OIDs or to enumerate SNMP tables exposed by a device. Reads are executed on demand and 
reflect the device state at query time.

### UPDATE <Tag description={"v26.1+"} />

`UPDATE` operations map to SNMP `SET` operations.

An UPDATE mutates the value of one or more writable OIDs on an existing managed instance. Conceptually, this represents **state mutation**, not entity modification.

Only OIDs defined as writable in the device MIB can be updated. Attempts to update read-only OIDs result in execution errors that are propagated back to the engine.

### Why INSERT and DELETE Are Not Supported

`INSERT` and `DELETE` operations are intentionally not supported by the SNMP Data Source.

This is not an engine limitation, but a direct consequence of the SNMP protocol model:

- SNMP does not define lifecycle operations for managed objects
- Managed instances exist independently of management access
- SNMP `SET` operations can modify values, but cannot create or remove managed instances

As a result, there is no protocol-level equivalent for `INSERT` or `DELETE`. 
Kubling preserves these semantics rather than introducing artificial abstractions that would misrepresent the underlying system.
...
WITH traffic_summary AS (
  SELECT
    ...
    ROUND(
      SUM(
        IFNULL(ifHCInOctets, 0) + IFNULL(ifHCOutOctets, 0)
      ) / 1073741824.0,
      2
    ) AS total_gb
  FROM snmpfull.ifXTable
  GROUP BY SUBSTRING(ifName, 1, 2)
)
SELECT *
...
<Callout type='warning'>
This query assumes `sysUpTime` is returned as a **numeric counter**. However, some appliances (such as certain Cisco switches) may return a **formatted string**, such as `82 days, 18:05:37.00`.
Make sure the field is castable to numeric, or handle device-specific variations using your own custom functions.
</Callout>
...
## Soft Transactions and SNMP Semantics

The SNMP Data Source can participate in Kubling Soft Transactions (STX), allowing SNMP operations to be coordinated with other 
data sources within a single transactional scope.

This participation, however, must be interpreted in the context of SNMP protocol semantics and device-specific behavior.

### Participation in Soft Transactions

When executed within a Soft Transaction, SNMP `UPDATE` operations are coordinated by the engine in the same way as operations targeting other data sources. 
Execution ordering, failure propagation, and transaction boundaries are handled consistently at the engine level.

This enables SNMP updates to be composed with other operations as part of a broader orchestration workflow.

### Rollback Semantics

SNMP does not provide native transactional support.

For SNMP updates executed under STX, Kubling performs rollback by issuing **compensation commands**. 
Conceptually, this means applying a follow-up SNMP `SET` intended to revert the managed value to its prior state.

A successful compensation command does not necessarily imply that the device has returned to the exact operational state that existed before the original update.

### Compensation Semantics and Operational Considerations

In Kubling, rollback for SNMP is always expressed through compensation: the engine derives and executes compensation commands as part of the rollback path.

The critical limitation is not whether Kubling can compute or execute a compensation command, 
but whether that compensation represents a meaningful and safe "undo" in the context of the specific device and managed OID.

In practice:

- An SNMP `SET` may trigger device-side side effects (state transitions, reconfiguration, asynchronous actions) that cannot be reversed 
by simply setting the previous value.
- The device state may evolve between the original update and the rollback attempt, making a direct revert semantically incorrect or operationally unsafe.
- Some managed values are better understood as control inputs than durable state, and reverting them may not restore previous behavior.

For these reasons, enabling SNMP updates within Soft Transactions requires domain understanding of:
- The device being managed
- The expected side effects of each `SET`
- Whether reverting a value is operationally valid within the lifecycle of the transaction

### Operational Implications

SNMP updates should be treated as **direct mutations of managed device behavior**, not as database-style row updates.

Soft Transactions provide coordination, ordering, and a structured rollback path through compensation, 
but they do not introduce stronger guarantees than those offered by the underlying protocol and the managed device’s behavior.

Kubling preserves these semantics intentionally, favoring correctness and transparency over artificial transactional abstractions.
