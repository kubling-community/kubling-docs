...
## Use Kubling DDL <Tag description={"v26.2+"} />

Instead of writing H2-specific DDL definitions, schemas can also be defined using native [Kubling DDL syntax](/engine/ddl).

This allows schema definitions to remain aligned with Kubling concepts and reduces dependency on database-specific syntax.
...
The referenced `full.sql` file must follow Kubling DDL syntax.

During initialization Kubling automatically translates the definition into the underlying storage schema required by the data source engine.

<Callout type='warning'>
At the moment Kubling DDL cannot be combined with schema migrations. This limitation is expected to be addressed in future releases.
</Callout>