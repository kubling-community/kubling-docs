...
| `TOBASE64(str)`    |  |    |
| `TOBASE64(str, encoding)`    |  |    |
| `FROMBASE64(str)`    |  |    |
| `FROMBASE64(str, encoding)`    |  |    |
| `COMPRESS(str)`    | Compresses a string using the Deflate algorithm with UTF-8 encoding. Returns the compressed output as a Base64-encoded clob.  |    |
| `COMPRESS(str, charset)`    | Compresses a string using the Deflate algorithm with UTF-8 encoding. Returns the compressed output as a Base64-encoded clob.  |    |
| `DECOMPRESS(str)`    | Decompresses (inflates) a Base64-encoded string using the Deflate algorithm, assuming UTF-8 as the charset. Returns the decompressed content as clob.  |    |
| `DECOMPRESS(str, charset)`    |   |    |
| `TOKENIZE(str, charset)`    | Extracts tokens of a string, using the specified delimiter, and returns an array.  |    |
...
| `TO_CHARS(blob, encoding)`              | Converts a `BLOB` value into a character representation using the specified encoding.                                 | `blob`, returns `clob`  |
| `TO_CHARS(blob, encoding, well_formed)` | Same as `TO_CHARS(blob, encoding)`, but allows control over malformed or invalid byte sequences during decoding.      | `blob`, returns `clob`  |
| `TO_BYTES(clob, encoding)`              | Converts a `CLOB` value into a binary representation using the specified encoding.                                    | `clob`, returns `blob`  |
| `TO_BYTES(clob, encoding, well_formed)` | Same as `TO_BYTES(clob, encoding)`, but allows control over malformed or invalid character sequences during encoding. | `clob`, returns `blob`  |
...
| **Function** | **Description** | **Datatype Constraint** |
|---|---|---|
| `MD5(str)` | Computes the MD5 hash of the specified plain text string. | `str` is `string`, returns `string` |
| `MD5(bin)` | Computes the MD5 hash of the specified binary value. | `bin` is `varbinary`, returns `string` |
| `HASH(str)` | Computes the SHA-256 hash of the specified plain text string and returns the result encoded as Base64. | `str` is `string`, returns `string` |
| `SHA1(str)` | Computes the SHA-1 hash of the specified plain text string. | `str` is `string`, returns `string` |
| `SHA1(bin)` | Computes the SHA-1 hash of the specified binary value. | `bin` is `varbinary`, returns `string` |
| `SHA2_256(str)` | Computes the SHA-256 hash of the specified plain text string. | `str` is `string`, returns `string` |
| `SHA2_256(bin)` | Computes the SHA-256 hash of the specified binary value. | `bin` is `varbinary`, returns `string` |
| `SHA2_512(str)` | Computes the SHA-512 hash of the specified plain text string. | `str` is `string`, returns `string` |
| `SHA2_512(bin)` | Computes the SHA-512 hash of the specified binary value. | `bin` is `varbinary`, returns `string` |
| `AES_ENCRYPT(x, y)` | Encrypts the specified value using the AES algorithm with `AES/CBC/PKCS5Padding` and the provided key. | `x` is `string` or `varbinary`, `y` is `string`, returns `varbinary` |
| `AES_DECRYPT(x, y)` | Decrypts the specified AES-encrypted binary value using the provided key. | `x` is `varbinary`, `y` is `string`, returns `varbinary` |
| `X509_CERT_EXP_DAYS(str)` | Returns certificate expiration information, in JSON format, for the specified X509 certificate. | `str` is `string`, returns `json` |
| `PKCS7_CERT_EXP_DAYS(str)` | Returns certificate expiration information, in JSON format, for the specified PKCS7 certificate. | `str` is `string`, returns `json` |
