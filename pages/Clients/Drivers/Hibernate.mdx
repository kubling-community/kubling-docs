...
# Hibernate
We [released a Hibernate dialect](https://github.com/kubling-community/kubling-teiid-os/tree/master/hibernate-dialect) that currently supports 
Hibernate `6.1.7`, but [we are planning](http://localhost:3000/roadmap) to move directly to version `7` once it is released, tested and adopted mostly by Spring and Quarkus.

<Callout>
  Take into account that from Hibernate version `6.1.x` to `6.2.x`, the `Dialect` base class changed some important methods like `initializeFunctionRegistry`,
