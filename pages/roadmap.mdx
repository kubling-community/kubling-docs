...
## Expose Kubling as an AI Agent to Provide Services to Other AI Agents and Copilots <Tag bgColor={"bg-kubling-dark-violet"} txtColor={"text-white"} description={"High Prio"} txtFont={"font-gilroyBold"} />
We are working on a way to embed the Kubling engine in an agentic environment. However, due to the complexity of the logic required, there will likely be a remote Kubling agent that the engine can call using an API token.  
This approach prevents the engine from having to handle complex LLM calls directly, in order to reserve the compute capacity for the main engine's task.

## Support SQL Functions from Modules in Queries <Tag bgColor={"bg-kubling-dark-violet"} txtColor={"text-white"} description={"High Prio"} txtFont={"font-gilroyBold"} />
Currently, SQL functions defined in [modules](/Modules/Functions/SQL) are only used to transform data at definition time (DDL). They cannot yet be invoked during query execution.
We are actively working on a mechanism that will enable these functions to be used directly in regular query execution.
