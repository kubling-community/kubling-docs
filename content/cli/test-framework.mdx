...
![Kubling integration test initialization flow](../../public/img/test-fw.svg)
...
<Callout type="info">
  External gRPC providers use the provider repository's
  [compatibility testing harness](https://github.com/kubling-community/kubling-providers/tree/main/testing)
  instead of the legacy embedded-source integration path described here.
</Callout>

The following self-contained manifest shows how the pieces fit together:
...
- **`yamlObjects`** this plays an important role which requires some extra explanation, [please see above](#virtual-database-and-data-sources-initialization).
...
|------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `executeQuery(query: string) : ListProxyObject<Row>` | Executes a `SELECT` clause and returns the list of [rows](test-framework#row) inside a [list](/modules/script-context-members#listproxyobject).                                                  |
...
04:39:41 - Generating Descriptor Module (/workspace/my-sample/descriptor-mod.zip)
04:39:41 - MockWebServer[8643] starting to accept connections
04:39:42 - Bundle file generated: /workspace/my-sample/descriptor-mod.zip
04:39:42 - Files included: 16 | Uncompressed: 20526 bytes (20 KB) | Compressed size: 9 KB
04:39:42 - Generating Main Module (/workspace/my-sample/main-mod.zip)
04:39:42 - Bundle file generated: /workspace/my-sample/main-mod.zip
