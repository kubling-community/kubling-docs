...
This delimiter is used to change the control flow of the template and to control the template context (variables).

### `include` <Tag description={"v26.2+"} />

The `include` tag allows a template to render another template inline as part of the current rendering process.

Included templates inherit the current rendering context and can therefore access the same variables available to the parent template.
...
authentication:
  scriptFilePath: "auth/authenticator_basic.js"

{% include "endpoints.yaml" %}
```

Included template paths are resolved relative to the current template location. This behavior works transparently for both filesystem-based templates and templates loaded from bundles.

Additional variables can be injected into the included template context using the `with` keyword:

```yaml
{% include "endpoints.yaml" with {"foo":"bar"} %}
```

In this case, the included template receives:
- The original rendering context
- The additional variables defined in the `with` map

### Variables

When the Engine calls the template parser, depending on the file, a context with a set of variables is injected.<br/>
By default, all contexts will contain the list of environment variables as well as variables defined in the [properties](/engine/properties).
...
![Template value flowing through a chain of parser filters](../../public/img/parser_filter_chain.svg)
