...
The client gRPC, native and PostgreSQL compatibility transports can each use
the same standard TLS configuration model while keeping independent
certificates and network policies.

When a secure connection is established, traffic exchanged between clients and
the Kubling instance is encrypted with `TLS 1.3`. Clients must still validate
the server certificate and protect their session credentials.
...
transports:
  ...
  grpc:
    ...
    secure: true
    sslConfig:
      storeFilePath: "/certs/server-grpc.ks"
      storePassword: "789ghi"
```
...
Filters are applied per protocol (`grpc`, `nativeProtocol`, `pgProtocol`) and
are defined using **CIDR blocks**, which can match single IPs or entire
subnets.
