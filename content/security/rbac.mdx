...
## Role Mappings
![Kubling role mappings between IAM roles and VDB data roles](../../public/img/rbac_roles.svg)
...
These roles are communicated via an authentication delegate, which is explained in the next section.
...
### Authentication Source Awareness <Tag description={"v26.2+"} />

Authentication delegates receive contextual information about how the authentication request reached the Engine.

This information is exposed through the `auth.authenticationSource` and `auth.httpAuthenticationType` context members, allowing the delegate to adapt authentication and authorization behavior depending on the transport and authentication mechanism involved.

For example, a delegate may:
- Accept `MTLS`-authenticated HTTP requests without password validation
- Apply different role mappings for API key–based requests
- Restrict privileged operations to native Engine transports
- Enforce distinct policies for interactive versus machine-to-machine authentication flows

This allows a single authentication delegate to centralize security decisions across multiple Engine entry points while preserving transport-specific semantics.

Let's explore the following example:
```js showLineNumbers copy
if (auth.authenticationSource === "HTTP") {

    // ------------------------------------------------------------
    // Machine-to-machine API authentication
    // ------------------------------------------------------------
    if (auth.httpAuthenticationType === "API_KEY") {

        const apiKey = auth.metadata["X-API-KEY"];

        if (apiKey === contextVars.INTERNAL_API_KEY) {
            auth.trust();
            auth.addRole("ROLE_API");
        } else {
            auth.bad("Invalid API key.");
        }

    // ------------------------------------------------------------
    // HTTP Basic authentication
    // ------------------------------------------------------------
    } else if (auth.httpAuthenticationType === "BASIC") {

        if (auth.userName === "reader" &&
            auth.credentials === "reader-pass") {
        ...
        auth.bad("Unsupported HTTP authentication mechanism.");
    }

} else if (auth.authenticationSource === "SOCKET_TRANSPORT") {

    // ------------------------------------------------------------
    // Native Engine transport authentication
    // ------------------------------------------------------------
    if (auth.userName === "sa") {
    ...
    auth.bad("Unknown authentication source.");
}
```
...
For more information about the `auth` object, see [here](/modules/script-context-members#authenticationauthorization-script).
