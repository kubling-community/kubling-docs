...
![DBeaver Libraries tab with the Kubling JDBC driver selected](../../../public/img/dbeaver/driver_loc.png)
...
![New Database Connection button in the DBeaver toolbar](../../../public/img/dbeaver/new_db_conn.png)

Filter `Driver` by name:

![Kubling driver selected in the DBeaver connection dialog](../../../public/img/dbeaver/driver_selection.png)

The connection string has the following components:<br/>
`jdbc:teiid:[vdb]@mm://[address]:[port]`

For a VDB named `MyVDB` running locally on the default native port, the
connection string is `jdbc:teiid:MyVDB@mm://localhost:35482`.

Credentials depend on the authentication delegate configured by that VDB. Do
not assume that username and password can be left blank.

The Quickstart sample cannot be used for this connection test because it
intentionally disables direct client transports and exposes only Kubling's
HTTP port for Studio.
