...
<Callout type="warning">
  `EMBEDDED_INMEM` keeps data inside the Kubling Engine process and is not
  recommended for large or durable datasets. When data volume can grow or its
  lifecycle must be independent from the Engine, use an
  [external provider-backed data source](/providers) instead. A
  provider designed as the natural replacement for this use case is on the
  roadmap and planned before the final Kubling release of 2026.
</Callout>
