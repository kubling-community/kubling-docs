...
- By default, `KublingContainer` uses the image selected by the library. You
  can pass another image explicitly, for example:
  ```java
  new KublingContainer<>(DockerImageName.parse("kubling/kubling:latest"))
  ```

  Rolling samples may intentionally follow `latest` under continuous
  validation. Tests that must reproduce a deployed environment should use the
  same immutable image tag as that deployment.

- To test a `PROVIDER_GRPC` data source, attach the provider container to the
  same Testcontainers network and register its network alias in the VDB.
  Native source credentials and connection settings belong to the provider,
  not to the Kubling container.
